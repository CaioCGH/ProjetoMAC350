...
\begin{tikzpicture}
  \node[entity] (node1) {Usuario}
  	child [grow=-10,level distance=3cm]{node[attribute] {us\_login}}
...
\paragraph{Restrições Entidade Regular:}
\begin{itemize}
  \item \textbf{us\_login:} Sequência de caracteres alfanuméricos e especiais \begin{verbatim}(!#$%&'*+-/=?^_`{|}~.@)\end{verbatim}. No mínimo de 4 e máximo de 80 caracteres. Não pode ser nulo.
  \item \textbf{us\_senha:}  Sequência de caracteres alfanuméricos e especiais \begin{verbatim}(!#$%&'*+-/=?^_`{|}~.@)\end{verbatim}. No mínimo de 8 e máximo de 80 caracteres. Não pode ser nulo.
  \item \textbf{us\_login:} Sequência de . No mínimo de 4 e máximo de 80 caracteres. Não pode ser nulo.
  \item \textbf{us\_username:} Sequência de letras maiúsculas e minúsculas, apóstrofe, hífen e espaço. No mínimo de 5 e máximo de 80 caracteres. Não pode ser nulo.
\end{itemize}
...
\begin{tikzpicture}
  \node[entity] (node1) {Oferecimento}
  	child [grow=-45,level distance=3cm]{node[attribute] {of\_DataInicio}}
  	child [grow=-90,level distance=3cm]{node[attribute] {of\_Nota}}
  	child [grow=-130,level distance=3cm]{node[attribute] {of\_Frequencia}}
  	child [grow=-150,level distance=3cm]{node[attribute] {of\_Horario}}
  	child [grow=-175,level distance=3cm]{node[attribute] {of\_Vagas}};
\end{tikzpicture}
